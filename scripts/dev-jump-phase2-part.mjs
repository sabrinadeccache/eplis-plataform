...
// `node scripts/dev-jump-phase2-part.mjs <email> <part1|part2|part3|part4> [itemIndex] [mode]`.
...
// `mode` (opcional, "practice" ou "official") — quando informado, só reaproveita
// (ou cria) uma tentativa daquele modo, mesmo que exista uma tentativa mais
// recente do OUTRO modo em andamento. Sem isso, uma tentativa antiga esquecida
// em `practice` podia ser reaproveitada por engano ao tentar testar `official`
// (achado real: aconteceu de verdade na máquina da Sabrina, 2026-08-11). Se
// omitido, mantém o comportamento antigo (pega a mais recente de qualquer modo;
// cria em `practice` se nenhuma existir).
...
async function main() {
  ...
  if (!email || !PART_INTRO_STATE[part]) {
    console.error(
      "Uso: node scripts/dev-jump-phase2-part.mjs <email> <part1|part2|part3|part4> [itemIndex] [practice|official]",
    );
    process.exit(1);
  }
  ...
  const { rows: attemptRows } = await client.query(
    `select id, mode from public.simulation_attempts
     where user_id = $1 and phase = 'phase2' and status = 'in_progress' ${modeFilter}
  ...
  if (attemptRows.length > 0) {
    ...
    const { rows: inserted } = await client.query(
      `insert into public.simulation_attempts
       ...
       values ($1, 'phase2', $2, 'in_progress', $3, $4, $5)
       returning id`,
