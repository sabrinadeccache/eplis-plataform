...
export async function advanceState(attemptId: string): Promise<{ finished: boolean }> {
  ...
  if (next === null) {
    const { data: responses } = await supabase
      ...
      .select(
        "id, transcript, response_stage, repetition_count, " +
    ...
    // falhar/estourar tempo, o resultado principal já está persistido e o
    // item fica só sem o feedback curto (a tela de resultado já trata null).
