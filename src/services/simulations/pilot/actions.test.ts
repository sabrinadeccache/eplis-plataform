...
vi.mock("@/lib/ai/pilot-track", () => ({
  generatePilotFinalReport: vi.fn(),
  generatePilotResponseFeedback: vi.fn(),
...
describe("generateSpeech (piloto) — proteção de custo", () => {
  ...
  it("rejeita tentativa de outra fase (ex.: Fase 2 do controlador)", async () => {
    attempt = { ...attempt!, phase: "phase2" };
    await expect(generateSpeech("attempt-1", "oi")).rejects.toThrow(
      "Tentativa inválida ou já finalizada.",
    );
    expect(generateSpeechAudio).not.toHaveBeenCalled();
